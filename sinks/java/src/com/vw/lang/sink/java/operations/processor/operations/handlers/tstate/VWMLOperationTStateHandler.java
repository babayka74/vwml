...
public class VWMLOperationTStateHandler extends VWMLOperationHandler {
	...
	protected VWMLEntity handleTStateOnComplexEntity(VWMLEntity entity, VWMLContext context, VWMLInterpreterImpl interpreter) throws Exception {
		...
		if (ts == null) {
			...
			Object timeHashId = VWMLEntity.buildHashIdFrom(String.valueOf(ts.getTime()));
			Object origTimeHashId = VWMLEntity.buildHashIdFrom(String.valueOf(ts.getOrigTime()));
			Object timeStampHashId = VWMLEntity.buildHashIdFrom(String.valueOf(ts.getTimeStamp()));
			// timer's remained time
			VWMLObject time = VWMLObjectBuilder.build(VWMLObjectType.SIMPLE_ENTITY, String.valueOf(ts.getTime()), timeHashId, context, 0, null);
			// start time
			VWMLObject origStartTime = VWMLObjectBuilder.build(VWMLObjectType.SIMPLE_ENTITY, String.valueOf(ts.getOrigTime()), origTimeHashId, context, 0, null);
			// last time stamp
			VWMLObject lastTimeStamp = VWMLObjectBuilder.build(VWMLObjectType.SIMPLE_ENTITY, String.valueOf(ts.getTimeStamp()), timeStampHashId, context, 0, null);
			...
			VWMLObjectsRepository.instance().lateBinding(result, result.buildReadableId());
		}
		return result;
	}	
}
